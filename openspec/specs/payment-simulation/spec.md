# payment-simulation Specification

## Purpose
A swappable `PaymentProcessor` interface with an in-process simulator implementation that produces deterministic approve/decline outcomes from a small set of test cards. The interface is the only entry point used by `lib/ordering`, so swapping to a real gateway later is a single binding change. The raw PAN, CVV, expiry, and cardholder name are accepted for the authorization call and then dropped — only `last4`, `brand`, `authorizationId`, and `amountCents` ever reach the database or logs.
