...
## Purpose
Server-authoritative cart pricing and order placement for both authenticated and guest customers. Cart totals are always recomputed server-side; orders are written as a single embedded MongoDB document so the lines, pricing, payment metadata, and event log are atomic. Guests receive a signed lookup token, and cross-customer reads return 404 (not 403) so order existence cannot be enumerated.
